...
class MappingMapperTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $repository = Mockery::mock(MyPharmaBr\Plastic\Mappings\Mappings::class);

        $fileSystem = Mockery::mock(Illuminate\Filesystem\Filesystem::class);

        $mapper = $this->getMock(MyPharmaBr\Plastic\Mappings\Mapper::class, ['resolve'], [$repository, $fileSystem]);
